...
function matches(country: employee, term: string, pipe: PipeTransform) {
  return country.id.toLowerCase().includes(term.toLowerCase())
  || country.name?.toLowerCase().includes(term.toLowerCase())
  || country.designationStr?.toLowerCase().includes(term.toLowerCase())
...
export class GridJsService {
  ...
  private _countries$ = new BehaviorSubject<employee[]>([]);
  ...
  set emps(employeelis:employee[]) { this.employeelist= employeelis; this._search$.next(); }
