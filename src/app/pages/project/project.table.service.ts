...
import { Project, projectSearchResult } from './models/project.model';
 ...
 */
function sort(tables: Project[], column: string, direction: string): Project[] {
  if (direction === '' || column === '') {
    return tables;
  } else {
 ...
 */
function matches(tables: Project, term: string) {
  if (!tables) {
    return false;
  }
  return (
    tables.name?.toLowerCase().includes(term) || tables.name?.includes(term)
...
export class ProjectTableService {
  ...
  // tslint:disable-next-line: variable-name
  private _tables$ = new BehaviorSubject<Project[]>([]);
  // tslint:disable-next-line: variable-name
   ...
   * Search Method
   */
  private _search(): Observable<projectSearchResult> {
