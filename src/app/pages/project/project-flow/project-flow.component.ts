...
try {
  ...
  getprimary(index){
    ...
    this.primary=index
    console.log('ethis.primary', this.primary)
  }
  collectionMCK:any;
...
this.RateArray=[{min:'30',rate: '65'},{min:'45',rate: '85'},{min:'60',rate: '100'}]
...
isEditGTC:boolean=false;

  // Transfer Lead Form (in modal)
  openEditGTC(transferLevelModal: any, lead: any, transferLevel: number = 1) {
...
this.isEditGTC=true;
    ...
    this.editGTCLead = lead;
    if (transferLevel > 1) {
      this.proratedValue = this.transferLead?.prorated;
    }
    console.log('this.transferLead', this.transferLead);
    this.EditGTCForm(alreadyGTC);
...
RateArray:any[] = [{min:'30',rate: '65'},{min:'45',rate: '85'},{min:'60',rate: '100'}]
//RateArray:any[] = []
GetRate(event,json){
...
onSubmit() {    
  const find_data = this.RateArray.find(item=>item.min === this.rateFormValues.min)
  if(find_data){
    find_data.rate = this.rateFormValues.rate;
    this.rateFormValues['min'+this.rateFormValues.min] = this.rateFormValues.rate;
  }else{
    this.RateArray.push({'min':this.rateFormValues.min,'rate':this.rateFormValues.rate})
  }
...
countries = [
  ...
  'China (Mainland)', 'Christmas Island', 'Cocos (Keeling) Islands', 'Colombia', 'Comoros', 'Congo, Republic of the',
  ...
  'France', 'French Guiana', 'French Polynesia', 'Gabon', 'Gambia, The', 'Georgia', 'Germany', 'Ghana', 'Gibraltar',
  'Greece', 'Greenland', 'Grenada', 'Guadeloupe', 'Guam', 'Guatemala', 'Guernsey', 'Guinea', 'Guinea-Bissau',
  'Guyana', 'Haiti', 'Holy See (Vatican City)', 'Honduras', 'Hong Kong SAR', 'Hungary', 'Iceland', 'India',
  'Indonesia', 'Iran', 'Iraq', 'Ireland', 'Israel', 'Italy', 'Jamaica', 'Jan Mayen', 'Japan', 'Jersey', 'Jordan',
  'Kazakhstan', 'Kenya', 'Kiribati', 'Korea, North', 'Korea, South', 'Kuwait', 'Kyrgyzstan', 'Laos', 'Latvia',
  'Lebanon', 'Lesotho', 'Liberia', 'Libya', 'Liechtenstein', 'Lithuania', 'Luxembourg', 'Macau (China)', 'Macedonia',
  'Madagascar', 'Malawi', 'Malaysia', 'Maldives', 'Mali', 'Malta', 'Man, Isle of', 'Marshall Islands', 'Martinique',
  ...
  'Republic of Kosovo', 'Reunion', 'Romania', 'Russia', 'Rwanda', 'Saint Kitts and Nevis', 'Saint Lucia',
  ...
  'United States', 'Uruguay', 'Uzbekistan', 'Vanuatu', 'Venezuela', 'Vietnam', 'Virgin Islands', 'Wallis and Futuna',
  'Western Sahara', 'Western Samoa', 'Yemen', 'Zaire (Dem Rep of Congo)', 'Zambia', 'Zimbabwe'
];
...
//new rate 30Jun2024
  ...
  // Given to Client
    ...
    let alreadyGTC = this.projectDetails?.givenToClient?.find((a) => a.id == this.editGTCLead.id);
    if (alreadyGTC) {
   ...
   // alreadyGTC.sme= this.projectDetails['givenToClient'] ? this.projectDetails['givenToClient']?.length + 1 : 1;
    alreadyGTC.relevance =fv.relevance;
    alreadyGTC.directInvolvement=fv.directInvolvement;
    alreadyGTC.canDiscuss=fv.canDiscuss;
    alreadyGTC.welcomeLines=fv.welcomeLines;
    alreadyGTC.additionalComments=fv.additionalComments;
    alreadyGTC.availability=fv.availability;
    alreadyGTC.conversationStyle=fv.conversationStyle;
    alreadyGTC.clientrates=this.RateArray;
    alreadyGTC.pricing=fv.pricing;
    alreadyGTC.isnew=this.isnewv; 
    alreadyGTC.updatedAt= new Date();
    alreadyGTC.rate = fv.rate
alreadyGTC.minDuration = fv.minDuration
alreadyGTC.minDurationType = fv.minDurationType
alreadyGTC.currency = fv.currency
alreadyGTC.rateType = fv.rateType
alreadyGTC.prorated = fv.prorated
alreadyGTC.CPrate = fv.CPrate
alreadyGTC.CPcurrency = fv.CPcurrency
alreadyGTC.CPrateType = fv.CPrateType
alreadyGTC.CPprorated = fv.CPprorated
alreadyGTC.CPminDuration = fv.CPminDuration
alreadyGTC.CPminDurationType = fv.CPminDurationType
    

    let alreadyGTCno = this.projectDetails?.givenToClient?.findIndex((a) => a.id == this.editGTCLead.id);
   
    this.projectDetails.givenToClient[alreadyGTCno]=alreadyGTC;

      //this.projectFlowData.givenToClient.push(gtcData);

      this.projectFlowData.givenToClient= this.projectDetails?.givenToClient

      this.updateOnServer(this.id, this.projectFlowData, 'update');
...
/*
  ...
  */
  ...
  private EditGTCForm(alreadyGTC) {
    this.transferExpertForm = this.formBuilder.group({
      welcomeLines: [null, [Validators.nullValidator]],
      relevance: [null, [Validators.nullValidator]],
      directInvolvement: [null, [Validators.nullValidator]],
    ...
    this.EditGTCFormValue(alreadyGTC);
  }


  EditGTCFormValue(alreadyGTC) {
    this.transferExpertForm.patchValue({
      rate: alreadyGTC.rate,
      currency: alreadyGTC.currency,
      rateType: alreadyGTC.rateType,
      minDuration:  alreadyGTC.minDuration,
      minDurationType:  alreadyGTC.minDurationType,
      prorated: this.proratedValue,
      welcomeLines: alreadyGTC?.welcomeLines,
      relevance: alreadyGTC?.relevance,
      directInvolvement: this.transferLead?.directInvolvement,
      canDiscuss: this.transferLead?.canDiscuss,
      availability: this.transferLead?.availability,
      conversationStyle: alreadyGTC?.conversationStyle,
      additionalComments: alreadyGTC?.additionalComments,
      CPrate: alreadyGTC.CPrate,
      CPminDuration: alreadyGTC.minDuration,
      CPcurrency: alreadyGTC.currency,
      CPminDurationType: alreadyGTC.CPminDurationType,
      CPrateType: alreadyGTC.CPrateType,
      CPmulFactor: alreadyGTC.CPmulFactor,
      // prorated: alreadyGTC?.prorated,
      clientrates:alreadyGTC.clientrates,
      CPprorated: alreadyGTC.CPprorated,
     
    });


    this.RateArray=alreadyGTC.clientrates;
  }
...
debugger
  ...
  eventForm() {
    ...
    clientapplicablerate: [null, [Validators.required]],
    ...
    this.eventFormGrp.patchValue({
      ...
      clientapplicablerate: this.lead?.clientrates,
      //   projectName: ,
     // "CPrateType" : "Hourly",
			//"prorated" : "Prorated",
...
console.log('this.workingDetails',this.workingDetails)
 ...
 for (let index = 0; index < this.workingDetails.length; index++) {
  ...
  console.log('element.network_employment_id',element.network_employment_id)


  if (element.network_employment_id==this.primary)
  {
...
if (element.company=="" || element.position=="" || element.responsibilities=="" || element.geography=="" || element.start_month=="" || element.start_year=="" )
{
  ...
  alert("Please fill all details for  employments");
  return
...
if(!this.geography || this.geography=="")
  {
    alert('please select a geographical rate card')
    return
  }

  if(!this.multipliertext || this.multipliertext?.multiplierValue =="")
    {
      alert('please select a multiplier Value')
      return
    }
