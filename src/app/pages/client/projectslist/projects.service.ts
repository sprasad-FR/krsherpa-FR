...
import {pSortColumn, pSortDirection} from './projects-sortable.directive';
...
function sort(countries: projects[], column: pSortColumn, direction: string): projects[] {
  if (direction === '' || column === '') {
    return countries;
  } else {
...
export class ProjectsTableService {
  ...
  set sortColumn(sortColumn: pSortColumn) { this._set({sortColumn}); }
  set sortDirection(sortDirection: pSortDirection) { this._set({sortDirection}); }
