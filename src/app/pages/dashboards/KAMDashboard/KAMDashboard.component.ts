...
@Component({
  selector: 'app-KAMDashboard',
  templateUrl: './KAMDashboard.component.html',
  styleUrls: ['./KAMDashboard.component.scss'],
...
export class KAMDashboardComponent implements OnInit {
  ...
  getIdsByManager(managerId) {
    console.log(managerId);
    const result = this.clientList.filter(obj => obj.keyaccmanagers?.includes(managerId)).map(obj => obj.id);
    console.log(result);
    return result;
  }
  ...
  */
  ...
  async getprjcount4cli() {


    var lst = this.getIdsByManager(this.whoaim?.id)
