...
export class HorizontalTopbarComponent implements OnInit {
  ...
  client: any;
  allroles: any=[];
  constructor(private router: Router,private arouter: ActivatedRoute,  public translate: TranslateService) {
  ...
  ngOnInit(): void {
    ...
    this.allroles=this.whoami.roles;
    console.log(this.allroles); // { orderby: "price" }
    this.arouter.queryParams
    .subscribe(params => {
      console.log(params['usr']); // { orderby: "price" }
    sessionStorage['usr']=params['usr'];
    //roles:any;
...
if (arr2=='all')
{
  ...
  const found = this.allroles.some(r=> arr2.indexOf(r) >= 0)
